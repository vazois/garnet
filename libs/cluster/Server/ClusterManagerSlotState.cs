...
namespace Garnet.cluster
{
    ...
    internal sealed partial class ClusterManager : IDisposable
    {
        ...
        public bool TryPrepareSlotForMigration(int slot, string nodeid, out ReadOnlySpan<byte> errorMessage)
        {
            ...
            logger?.LogInformation("MIGRATE {slot} TO {currentConfig.GetWorkerAddressFromNodeId(nodeid)}", slot, currentConfig.GetWorkerDataEndpoint(nodeid));
            return true;
        }
        ...
        public bool TryPrepareSlotsForMigration(HashSet<int> slots, string nodeid, out ReadOnlySpan<byte> errorMessage)
        {
            ...
            logger?.LogInformation("MIGRATE {slot} TO {migrating node}", string.Join(' ', slots), currentConfig.GetWorkerDataEndpoint(nodeid));
            return true;
        }
        ...
        public bool TryPrepareSlotForImport(int slot, string nodeid, out ReadOnlySpan<byte> errorMessage)
        {
            ...
            logger?.LogInformation("IMPORT {slot} FROM {currentConfig.GetWorkerAddressFromNodeId(nodeid)}", slot, currentConfig.GetWorkerDataEndpoint(nodeid));
            return true;
        }
        ...
        public bool TryPrepareSlotsForImport(HashSet<int> slots, string nodeid, out ReadOnlySpan<byte> errorMessage)
        {
            ...
            logger?.LogInformation("IMPORT {slot} FROM {importingNode}", string.Join(' ', slots), currentConfig.GetWorkerDataEndpoint(nodeid));
            return true;
        }
        ...
        public bool TryPrepareSlotForOwnershipChange(int slot, string nodeid, out ReadOnlySpan<byte> errorMesage)
        {
            ...
            if (current.GetState((ushort)slot) is SlotState.MIGRATING)
            {
                ...
                FlushConfig();
                logger?.LogInformation("SLOT {slot} IMPORTED TO {nodeid}", slot, currentConfig.GetWorkerDataEndpoint(nodeid));
                return true;
            }
            else if (current.GetState((ushort)slot) is SlotState.IMPORTING)
            {
                ...
                FlushConfig();
                logger?.LogInformation("SLOT {slot} IMPORTED FROM {nodeid}", slot, currentConfig.GetWorkerDataEndpoint(nodeid));
                return true;
            }
            return true;
        }
        ...
        public bool TryPrepareSlotsForOwnershipChange(HashSet<int> slots, string nodeid, out ReadOnlySpan<byte> errorMessage)
        {
            ...
            FlushConfig();
            logger?.LogInformation("SLOT {slot} IMPORTED TO {endpoint}", slots, currentConfig.GetWorkerDataEndpoint(nodeid));
            return true;
        }
