...
{
    public class GarnetClusterTcp : GarnetServerBase, IServerHook
    {
        ...
        public GarnetClusterTcp(string address, int port, int networkBufferSize = default, IGarnetTlsOptions tlsOptions = null, int networkSendThrottleMax = 8, ILogger logger = null)
        ...
        private unsafe bool HandleNewConnection(SocketAsyncEventArgs e)
        {
            ...
            ClusterTcpNetworkHandler handler = null;
            if (activeHandlerCount >= 0)
            {
                if (Interlocked.Increment(ref activeHandlerCount) > 0)
                {
                    try
                    {
                        handler = new ClusterTcpNetworkHandler(this, e.AcceptSocket, networkPool, tlsOptions != null, networkSendThrottleMax, logger);
        ...
        /// <summary>
        /// Create cluster session (message consumer) given incoming bytes
        /// </summary>
        ...
        public bool TryCreateMessageConsumer(Span<byte> bytes, INetworkSender networkSender, out IMessageConsumer session)
        {
            ...
            if (!AddSession(protocol, MessageConsumerType.ClusterSessionConsumer, ref provider, networkSender, out session))
